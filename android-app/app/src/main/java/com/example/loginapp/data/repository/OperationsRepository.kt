...
class OperationsRepository(private val authRepository: AuthRepository) {
    ...
    private val apiService = RetrofitClient.create(authRepository)

    suspend fun getOpenEvents(): Result<List<Event>> {
        return safeCall { handleResponse(apiService.getEvents("OPEN")) }
    }
